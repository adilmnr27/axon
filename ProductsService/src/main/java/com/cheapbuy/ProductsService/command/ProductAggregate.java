...
public class ProductAggregate {
	...
	/**
	 * Will be used to initialize the current state of the Aggregate with the latest information. 
	 * 
	 */
	 ...
	 * @param createProductCommand The dispatched command which will invoke this function
	 */
	@CommandHandler
	public ProductAggregate(CreateProductCommand createProductCommand) {
		...
		// handlers (here know as event source handler) inside this aggregate, so that state of Aggregate can be updated
	 ...
	 * Just "on" Handles the event raised by our aggregate. (Events are dispatched by calling the apply method) <br>
	 * Will be used to initialize the current state of the Aggregate with the latest information. <br>
	 * Hence we would require instance variables related to product information. <br>
	 * Should avoid business logic and only use it to update Aggregate state <br>
	 * 
	 * productCreatedEvent after this method will be scheduled for publication to other event handler
	 * and then persisted in event store <br>
	 */
